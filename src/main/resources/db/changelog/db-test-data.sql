...
WHERE et.name = 'Youth Theatre' AND l.name = 'Tillicoultry'
  AND NOT EXISTS (
    ...
    WHERE ei.event_type_id = et.id
      AND ei.start_time = TIMESTAMP '2026-03-30 14:30:00'
);
...
WHERE et.name = 'Walk'
  AND ei.start_time = TIMESTAMP '2026-03-31 17:30:00'
  AND NOT EXISTS (
    SELECT 1 FROM event_assignment ea
    WHERE ea.event_instance_id = ei.id AND ea.staff_id = s.id
);
...
JOIN staff s ON s.email = 'bill.jones@oyci.org'
WHERE et.name = 'Walk'
  AND ei.start_time = TIMESTAMP '2026-03-30 17:30:00'
  AND NOT EXISTS (
    SELECT 1 FROM event_assignment ea
    WHERE ea.event_instance_id = ei.id AND ea.staff_id = s.id
);
...
-- ============================================================
--  Additional staff  (6 new members)
--  John, Angelina, David  → ASC qualification
--  Sarah, Marcus, Priya   → random / no specific qualification
-- ============================================================
...
-- ------------------------------------------------------------
INSERT INTO staff (name, email, weekly_avail_hours, experience_months, designation)
SELECT 'John Campbell', 'john.campbell@oyci.org', 30, 24, 'Youth Worker'
WHERE NOT EXISTS (SELECT 1 FROM staff WHERE email = 'john.campbell@oyci.org');

INSERT INTO staff (name, email, weekly_avail_hours, experience_months, designation)
SELECT 'Angelina Murray', 'angelina.murray@oyci.org', 25, 18, 'Session Support Worker'
WHERE NOT EXISTS (SELECT 1 FROM staff WHERE email = 'angelina.murray@oyci.org');

INSERT INTO staff (name, email, weekly_avail_hours, experience_months, designation)
SELECT 'David Stewart', 'david.stewart@oyci.org', 35, 36, 'Youth Worker'
WHERE NOT EXISTS (SELECT 1 FROM staff WHERE email = 'david.stewart@oyci.org');

INSERT INTO staff (name, email, weekly_avail_hours, experience_months, designation)
SELECT 'Sarah Thomson', 'sarah.thomson@oyci.org', 20, 12, 'Session Support Worker'
WHERE NOT EXISTS (SELECT 1 FROM staff WHERE email = 'sarah.thomson@oyci.org');

INSERT INTO staff (name, email, weekly_avail_hours, experience_months, designation)
SELECT 'Marcus Reid', 'marcus.reid@oyci.org', 28, 6, 'Admin'
WHERE NOT EXISTS (SELECT 1 FROM staff WHERE email = 'marcus.reid@oyci.org');

INSERT INTO staff (name, email, weekly_avail_hours, experience_months, designation)
SELECT 'Priya Sharma', 'priya.sharma@oyci.org', 32, 30, 'Youth Worker'
WHERE NOT EXISTS (SELECT 1 FROM staff WHERE email = 'priya.sharma@oyci.org');

-- ------------------------------------------------------------
-- staff_qualification
-- John, Angelina, David → ASC qualification
-- ------------------------------------------------------------
INSERT INTO staff_qualification (staff_id, qualification_id)
SELECT s.id, q.id
FROM staff s, qualification q
WHERE s.email = 'john.campbell@oyci.org' AND q.name = 'ASC'
  AND NOT EXISTS (
    SELECT 1 FROM staff_qualification sq
    WHERE sq.staff_id = s.id AND sq.qualification_id = q.id
);

INSERT INTO staff_qualification (staff_id, qualification_id)
SELECT s.id, q.id
FROM staff s, qualification q
WHERE s.email = 'angelina.murray@oyci.org' AND q.name = 'ASC'
  AND NOT EXISTS (
    SELECT 1 FROM staff_qualification sq
    WHERE sq.staff_id = s.id AND sq.qualification_id = q.id
);
...
SELECT s.id, q.id
FROM staff s, qualification q
WHERE s.email = 'david.stewart@oyci.org' AND q.name = 'ASC'
  AND NOT EXISTS (
    SELECT 1 FROM staff_qualification sq
    WHERE sq.staff_id = s.id AND sq.qualification_id = q.id
);
...
-- ------------------------------------------------------------
-- staff_qualification
-- Existing staff (Joe, Bill, Kirsty, Daniel, Sarah, Marcus, Priya) → Drama qualification
-- ------------------------------------------------------------
INSERT INTO staff_qualification (staff_id, qualification_id)
SELECT s.id, q.id
FROM staff s, qualification q
WHERE s.email = 'joe.smith@oyci.org' AND q.name = 'Drama'
  AND NOT EXISTS (
    SELECT 1 FROM staff_qualification sq
    WHERE sq.staff_id = s.id AND sq.qualification_id = q.id
);
...
SELECT s.id, q.id
FROM staff s, qualification q
WHERE s.email = 'bill.jones@oyci.org' AND q.name = 'Drama'
  AND NOT EXISTS (
    SELECT 1 FROM staff_qualification sq
    WHERE sq.staff_id = s.id AND sq.qualification_id = q.id
);
...
SELECT s.id, q.id
FROM staff s, qualification q
WHERE s.email = 'kirsty.macdonald@oyci.org' AND q.name = 'Drama'
  AND NOT EXISTS (
    SELECT 1 FROM staff_qualification sq
    WHERE sq.staff_id = s.id AND sq.qualification_id = q.id
);
...
SELECT s.id, q.id
FROM staff s, qualification q
WHERE s.email = 'daniel.robertson@oyci.org' AND q.name = 'Drama'
  AND NOT EXISTS (
    SELECT 1 FROM staff_qualification sq
    WHERE sq.staff_id = s.id AND sq.qualification_id = q.id
);
...
SELECT s.id, q.id
FROM staff s, qualification q
WHERE s.email = 'sarah.thomson@oyci.org' AND q.name = 'Drama'
  AND NOT EXISTS (
    SELECT 1 FROM staff_qualification sq
    WHERE sq.staff_id = s.id AND sq.qualification_id = q.id
);
...
SELECT s.id, q.id
FROM staff s, qualification q
WHERE s.email = 'marcus.reid@oyci.org' AND q.name = 'Drama'
  AND NOT EXISTS (
    SELECT 1 FROM staff_qualification sq
    WHERE sq.staff_id = s.id AND sq.qualification_id = q.id
);
...
SELECT s.id, q.id
FROM staff s, qualification q
WHERE s.email = 'priya.sharma@oyci.org' AND q.name = 'Drama'
  AND NOT EXISTS (
    SELECT 1 FROM staff_qualification sq
    WHERE sq.staff_id = s.id AND sq.qualification_id = q.id
);
